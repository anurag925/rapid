...
type (
	// Client is that client that allows you to queue or schedule task execution
	Client struct {
...
func WithClient(conn asynq.RedisConnOpt) func(*Client) {
...
func WithSchedular(conn asynq.RedisConnOpt, opts *asynq.SchedulerOpts) func(*Client) {
	return func(s *Client) {
		s.scheduler = asynq.NewScheduler(conn, opts)
	}
}

func WithServer(conn asynq.RedisConnOpt, cfg asynq.Config) func(*Client) {
	return func(s *Client) {
		s.server = asynq.NewServer(conn, cfg)
	}
}
...
func (t *Client) StartScheduler() error {
...
// Starts the asynq server and starts performing the jobs
func (t *Client) AddTask(pattern string, handler asynq.Handler) {
	t.mux.Handle(pattern, handler)
}

// New starts a task creation operation
func (t *Client) New(typ string) *task {
	return &task{
		client: t,
		typ:    typ,
	}
